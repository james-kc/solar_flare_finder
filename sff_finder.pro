...
pro sff_finder, run_offline = run_offline
  ...
  if keyword_set( run_offline ) then begin
    ...
    top_dir = '~/solar_flare_finder/test'
    !p.charsize = 0.8
    thisDevice = !D.Name
    Set_Plot, 'Z'
    ...
    ;!p.background = 255
    ;!p.color = 0
    device, Set_Resolution=[ 1000, 500 ]
  endelse
...
;; automatically update the file.  
  sock_copy, 'http://lasp.colorado.edu/eve/data_access/evewebdata/interactive/megsb_daily_exposure_hours.csv', $
    'megsb_daily_exposure_hours.dat', out_dir = top_dir, /clobber
  readcol, '~/eve_data/megsb_daily_exposure_hours.dat', date, doy, exp, delim = ',', skipline = 1, format = 'A,I,A'
...
;; Overplot GOES15 EUVS Lya lightcurve       
      ...
      endelse
       ...
       legend, [ 'GOES Start', 'GOES Peak', 'GOES End' ], line = [ 1, 0, 2 ], color = 80, /top, /right, box = 0, thick = 2.
       legend, gev[ i ].goes_class, /left, /top, box = 0
       ...
       if ( eis_obs eq 1 ) then begin
         for k = 0, n_elements( eis_fov_check )-1 do begin
           evt_grid, anytim2utc( eis_rasters[ eis_fov_check[ k ] ].date_obs, /vms ), line = 1, color = 3, /ticks, ticklen = 0.05,  tickpos = 0.68, thick = 2
           evt_grid, anytim2utc( eis_rasters[ eis_fov_check[ k ] ].date_end, /vms ), line = 2, color = 3, /ticks, ticklen = 0.05, tickpos = 0.68, thick = 2.
           ...
           legend, [ 'EIS Start', 'EIS end' ], line = [ 1, 2  ],  position = [ 0.47, 0.86 ], /norm, color = 3, box = 0, /right
         endfor
       endif  
       if ( sot_obs eq 1 ) then begin
         for k = 0, sot_count-1 do begin
           ...
           legend, 'SOT', position= [ 0.38, 0.79 ], /norm, textcolor = 5, box = 0
         endfor
       endif  
       if ( xrt_obs eq 1 ) then begin
         for k = 0, xrt_count-1 do begin
           ...
           legend, 'XRT', position= [ 0.38, 0.81 ], /norm, textcolor = 6, box = 0
         endfor
         endif
         if ( eve_aobs eq 1 ) then begin
          ...
          legend, 'EVE MEGS-A', textcolor = 7, box = 0, position = [ 0.38, 0.75 ], /norm
       endif   
       if ( eve_bobs eq 1 ) then begin
          ...
          legend, 'EVE MEGS-B', textcolor = 4, box = 0, position = [ 0.38, 0.73 ], /norm
       endif   
       if ( iris_obs eq 1 ) then begin
          ...
          legend, 'IRIS', textcolor = 2, box = 0, position = [ 0.38, 0.77 ], /norm
       endif   
...
;; Plot RHESSI lightcurves       
       ...
       position = [ 0.05, 0.08, 0.46, 0.47 ], flag_colors = [ 7, 6, 2 ], ytickformat = 'tick_label_exp', yrange = [ 1e-1, 1e6 ], ystyle = 1, dim1_use = indgen( max_ind )+1
       legend, hsi_labels[ 0:max_ind ], /top, /left, box = 0, charsize = 1., textcolor = [ 0, indgen( max_ind )+2 ]
       evt_grid, gev_start[ i ], line = 1, color = 80, thick = 2.
       evt_grid, gev_peak[ i ], line = 0, color = 80, thick = 2.
       evt_grid, gev_end[ i ], line = 2, color = 80, thick = 2.
       !p.multi = [ 1, 2, 1 ]
...
;; create map structure. If no AIA, use SWAP or XRT.
    ...
    if ( vaia_ind[ 0 ] ne -1 ) then sock_copy, vaia[ vaia_ind[ 0 ] ], local = local, /verbose, out_dir = top_dir+'/aia_files'
      if ( local eq '' ) then begin
        vswap = vso_files( anytim( gev_peak[ i ], /vms ), anytim( gev_peak[ i ]+600., /vms ), inst = 'swap', /url )
        sock_copy, vswap[ 0 ], local = local, /verbose, out_dir = top_dir+'/swap_files'
        if ( vswap[ 0 ] eq '' ) or ( local eq '' ) then begin
          ...
          if ( vxrt[ 0 ] ne '' ) then begin
            sock_copy, vxrt[ 0 ], local = local, /verbose, out_dir = top_dir+'/xrt_files'
            fits2map, local, map
            loadct, 3
            chg_ctable, gamma = 1.
          endif  
...
;; Plot map
   ...
   if ( hsi_obs eq 0 ) then legend, 'AIA flare location', color = 255, textcolor = 255, psym = 8, /top, /left, box = 0 else legend, [ 'AIA flare location', 'RHESSI contours' ], psym = 8, color = [ 255, 0 ], /top, /left, box = 0
...
;; Overplot EIS FOV       
         if ( eis_obs eq 1 ) then begin
           for p = 0, n_elements( eis_fov_check )-1 do begin
             ...
             legend, 'EIS FOV', textcolor = 3, position = [ -1100, -800 ], /data, box = 0
           endfor
        endif
...
;; Overplot SOT FOV       
         if ( sot_obs eq 1 ) then begin
           for q = 0, n_elements( sot_fov_check )-1 do begin
             ...
             legend, 'SOT FOV', textcolor = 5, position = [ -1100, -850 ], /data, box = 0
           endfor
        endif
...
;; Overplot IRIS FOV       
         if ( iris_obs eq 1 ) then begin
           for s = 0, n_elements( iris_fov_check )-1 do begin
             ...
             legend, 'IRIS FOV', textcolor = 2, position = [ -1100, -950 ], /data, box = 0
          endfor
         endif
       ...
       if keyword_set( run_offline ) then begin
         image = TVRD( /true )
         TVLCT, r, g, b, /Get
         write_png, top_dir+'/png_files/'+fid+'.png', tvrd( /true )
       endif else begin   
         image = TVRD()
         TVLCT, r, g, b, /Get
         write_png, top_dir+'/png_files/'+year+'/'+month+'/'+fid+'.png', image, r, g, b
       endelse
       ...
       sff_str = { sol:sol, fid:fid, gev:gev[ i ], hsi_flare:hsi_flare, eve_megsa_times:megsa_times, eve_megsb_times:megsb_times, $
                     eis_rasters:eis_rasters, sot_data:sot_data, xrt_data:xrt_data, iris_rasters:iris_rasters }
                                ;save, filename =
                                ;top_dir+'/sav_files/'+year+'/'+month+'/'+fid
                                ;+ '.sav', sff_str
       stop
